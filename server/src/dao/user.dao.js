  ...
  findPublicById(id) {
    return db.queryOne(
      `SELECT id, username, nickname, avatar, bio, created_at AS createdAt,
        ...
        (SELECT COUNT(*) FROM follows f1 WHERE f1.following_id = users.id) AS followerCount,
        (SELECT COUNT(*) FROM follows f2 WHERE f2.follower_id = users.id) AS followingCount
